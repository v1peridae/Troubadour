...
<header class="fixed top-2 left-0 w-full z-10">
    ...
    <h4 id="username" class="text-2xl fixed right-1/8 top-6 text-cyan-950 ">{username}</h4>
    <a id="signout" class="fixed text-xl right-5 top-3 text-cyan-800 border-2 border-rose-700 rounded-2xl p-3">Sign-out</a>
</header>
...
{/if}
-->
<div id="main-container" class="border-black border-4 rounded-xl h-110 w-7/8 self-center mt-6 ml-auto mr-auto">
    {#each lyrics as song}
        <LyricTile title={song.title} artist={song.artist} album_url={song.album_url ? song.album_url : "missing_album_src"} album_name={song.album_name} />
    {/each}
</div>